...
class BookingForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'special_requests': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 4,
                'placeholder': _('Особые пожелания, требования к размещению...')
            }),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.cottage:
            ...
            self.fields['guests'].widget.attrs['placeholder'] = _('Максимум {capacity} гостей').format(capacity=self.cottage.capacity)
    ...
    def clean_check_in(self):
        ...
        if check_in:
            ...
            if check_in > max_date:
                raise ValidationError(_('Бронирование возможно максимум на год вперед'))
    ...
    def clean_check_out(self):
        ...
        if check_out:
            if check_out <= date.today():
                raise ValidationError(_('Дата выезда должна быть в будущем'))
            ...
            if check_in:
                ...
                if duration > 30:
                    raise ValidationError(_('Максимальная продолжительность бронирования - 30 дней'))
                if duration < 1:
                    raise ValidationError(_('Минимальная продолжительность - 1 день'))
    ...
    def clean_guests(self):
        ...
        if guests:
            if guests < 1:
                raise ValidationError(_('Количество гостей должно быть больше 0'))
            
            if self.cottage and guests > self.cottage.capacity:
                raise ValidationError(
                    _('Максимальная вместимость коттеджа: {capacity} гостей').format(capacity=self.cottage.capacity)
                )
    ...
    def clean(self):
        ...
        if self.cottage and check_in and check_out:
            ...
            if conflicting_bookings.exists():
                raise ValidationError(
                    _('Выбранные даты недоступны. Пожалуйста, выберите другие даты.')
                )
